<script>
    import Buttons from "./components_facturas/buttons.svelte"
    ...
    onMount(async () => {
        ...
        const {data: medico, error: medico_error} = await supabase
            ...
            const {data: factura, error: error_factura} = await supabase
                .rpc('medico_facturas_consultar', {pvalor: "a", id: medico[0].medico_id})
    ...
    async function handleSearch(event){
        try{
            const {data: medico, error: medico_error} = await supabase
                ...
                const {data: factura, error: error_factura} = await supabase
                    .rpc('medico_facturas_consultar', {pvalor: searchValue, id: medico[0].medico_id})
                    ...
                    <tbody>
                        {#each facturas as factura}
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="px-4 py-3">{factura.paciente}</td>
